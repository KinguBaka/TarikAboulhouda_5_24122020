...
function afficherNounoursNameParagraphe(nounours) { 
    for (let elem of nounours) {
        ...
        let newContent = document.createTextNode(elem.name);
        newP.appendChild(newContent);
        let currentP = document.getElementById("nounours");
        document.body.insertBefore(newP, currentP);
    }
};

//afficherNounoursNameParagraphe(nounourse);


function afficherDivAvecNomEtPrixNounours(nounours) {
    for (let elem of nounours) {
        let newDiv = document.createElement("div");
        let newContent = document.createTextNode(elem.name + " " + elem.price);
        newDiv.appendChild(newContent);
        let currentDiv = document.getElementById("nounoursDiv");
        document.body.insertBefore(newDiv, currentDiv);
    }
};

//afficherDivAvecNomEtPrixNounours(nounourse);

function afficherPhotoDesNounours(nounours) {
    for (let elem of nounours) {
        let newImg = document.createElement("img");
        newImg.setAttribute("src", elem.imageUrl);
        let currentDiv = document.getElementById("nounoursImg");
        document.body.insertBefore(newImg, currentDiv);
    }
};

//afficherPhotoDesNounours(nounourse);
...
function afficherLeToutSousFormeDeDivision(nounours) {
    for (let elem of nounours) {
        ...
        document.body.insertBefore(newNournours, document.getElementById("produits"))   //Div placé avant la div avec l'id "produits"
